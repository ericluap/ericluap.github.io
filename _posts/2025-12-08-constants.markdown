---
layout: post
title: "Tracking Constants"
date: 2025-12-08
categories: lean
---

The big idea is that Lean has a hashmap from the names of previously defined constants to their definitions. When it encounters a constant, it looks up what the constant means inside of this hashmap. Very reasonable.
...
## The staged map
It turns out that Lean doesn't actually store all the constants we've defined in a single hashmap. It splits things into two hashmaps! The first hashmap stores all the constants defined in stuff that we're importing while the second hashmap stores all the constants we're defining locally. And so `String` is in the first hashmap and `test` is in the second. Somehow our environment conversions are losing everything in the second hashmap of local constants.

But wait why does Lean split things up into two hashmaps? The answer is speed and the key is that the second hashmap is not actually a hashmap but a hash trie.
...
In the current file, each definition has to be fully processed before adding itself to the list of constants and that's why Lean does all this asynchronous stuff. However, the asynchronous things happening means that there is more than one reference to the hashmap storing these constants and so using `Std.HashMap` would be slow. That's where the `PersistentHashMap` comes into play. The way it is implemented is called a hash array mapped trie and it looks like a shallow and wide tree. Insertions just have to update a single path in the tree and all the rest of tree can be reused. Thus, when there are many references to the hashmap and things need to be copied, the `PersistentHashMap` is faster.

Ok so we understand that Lean stores imported constants in one hashmap and locally defined constants in another in order to be fast. But the question still remains: why does going from `Environment` to `Kernel.Environment` back to `Environment` lose all the constants in the hashmap used for locally defined constants?

## Finding a constant in the maps
When you have a `Kernel.Environment` and try to look for a constant in its two hashmaps, Lean checks for the constant in both of them. However, if you have an `Environment` there might be constants that have been defined but are still being processed asynchronously. In order to handle this, Lean has yet another hashmap! This hashmap is called `asyncConstsMap`. Each definition first adds itself immediately to the `asyncConstsMap` with its value being a promise (accessing a promise blocks until the asynchronous processing is completed). And then once the processing is complete, the constant is added to the hashmap in the `Kernel.Environment` that stores locally defined constants.

So when we try to find a constant in the `Environment`, it first checks the `Kernel.Environment` that it has built up so far for the constant and then checks to see if the constant is inside `asyncConstsMap`. But every constant that ends up in the second hashmap in `Kernel.Environment` first was added to `asyncConstsMap`. So in order to avoid looking through these constants twice, Lean does not check the second hashmap of the `Kernel.Environment`.

And so there is root of our issue. Lean assumes that every locally defined constant that appears in the second hashmap in its current `Kernel.Environment` also appears in `asyncConstsMap`. When we turn our `Environment` into a `Kernel.Environment` and then back into an `Environment`, we lose everything inside of `asyncConstsMap` and break that invariant.
...
run_meta
  ...
  Lean.logInfo m!"{info.type}"
```
If we look for `test` inside of `kernelEnv` it succeeds because it checks both hashmaps. So indeed the final `env` knows about `test` but fails to find it due to the broken invariant.
